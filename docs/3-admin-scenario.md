...
- 범위 밖(미정): Admin이 타 회원의 Todo·회원정보를 직접 수정·삭제하는 기능, 기본 Category 자체의 삭제/수정 정책은 PRD 3.2 확장 범위 및 도메인 정의서 9장 미정 항목으로 본 문서에서 다루지 않는다.
...
**사전조건(Precondition)**
- Role이 Admin인 User가 로그인한 상태다.
- 삭제 대상 Category는 기본(isDefault=true) Category가 아니다(기본 Category 삭제 정책은 범위 밖, 미정).
...
- 삭제 대상이 기본 Category인 경우: 삭제 불가(정책 미확정, 범위 밖) — 현재는 기능 자체를 제공하지 않음.
...
- 전역 '기본' Category 자체의 삭제/수정 가능 여부 확정
